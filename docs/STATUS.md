...
**로또팟 v1.0.0 — 지인 배포 가능한 상태.** 다섯 탭 모두 구현, 정식 키로 서명,
이름·아이콘 적용 완료. 무인 갱신도 실제로 한 바퀴 돌았다.
지금은 **지인 피드백을 기다리는 중**이며, 수정 요청이 오면
아래 '피드백 받고 고칠 때'부터 읽으면 된다.
...
| | |
|---|---|
| 버전 | **v1.0.0** (`pubspec.yaml` 의 `version: 1.0.0+1`) |
| 배포 파일 | `app/build/app/outputs/flutter-apk/app-arm64-v8a-release.apk` (23.1MB) |
| 사본 | 바탕화면 `로또팟_v1.0.0.apk` |
| 예비 | `app-armeabi-v7a-release.apk` (19MB) — 구형 기기용 |
| 서명 | `CN=Jongho HA, OU=HJH, O=HJH, C=KR` (디버그 키 아님, `DEBUGGABLE` 없음) |
| 배포처 | 지인 직접 전달. Play 미등록 |

- `build/` 의 낡은 산출물(디버그 APK·이름 바뀌기 전 릴리스·AAB·x86_64)은
  2026-08-09에 지웠다. 전부 명령 한 줄로 다시 만들 수 있는 것들이다
- **`app-release.apk`(통합 61MB)를 다시 만들면 배포에 쓰지 말 것.**
  그중 22MB가 에뮬레이터 전용(x86_64)이다. `--split-per-abi` 를 쓸 것

### 신규 회차가 앱에 들어오는 경로

```
토 20:45  추첨
   ↓  배치가 토 21:00~22:59 KST 10분 간격으로 실행 (+ 일 09:00·12:00)
...
## 피드백 받고 고칠 때 (다음 세션이 가장 먼저 볼 곳)

지인에게 `로또팟 v1.0.0`(arm64 APK)을 나눠준 상태다. 수정 요청이 오면 이 순서다.

### 1) 고친다

평소처럼 코드를 고치고 `flutter test` · `flutter analyze` 를 돌린다.

### 2) 버전을 올린다 — **빠뜨리면 업데이트가 안 된다**

`app/pubspec.yaml` 의 `version: 1.0.0+1` 을 올린다.

```yaml
version: 1.0.1+2      # 표시용 이름 + 빌드 번호
```

**`+` 뒤 숫자(빌드 번호)가 반드시 커져야** 받는 사람 폰이 '업데이트'로 인식한다.
그대로 두면 "이미 설치된 앱이 있습니다"로 막힌다.

`--split-per-abi` 는 여기에 접두어를 붙인다(arm64 → `2002`). 자동이라 신경 쓸 것 없다.
...
Copy-Item "build\app\outputs\flutter-apk\app-arm64-v8a-release.apk" `
          "$env:USERPROFILE\Desktop\로또팟_v1.0.1.apk"
```

### 4) 받는 사람은 그냥 덮어 설치하면 된다

**v1.0.0과 같은 키로 서명되므로 지우지 않아도 된다.** 저장해둔 내 번호도 남는다.
(서명이 다르면 지우고 새로 깔아야 하는데, 그 일은 이제 없다 —
`android/key.properties` 와 `lotto-release.jks` 를 잃지 않는 한.)

### 자주 나올 만한 요청과 손댈 곳

| 요청 | 어디를 고치나 |
|---|---|
| 구형 폰에서 켜자마자 꺼진다 | **API 26 호환성.** 아래 '아직 안 한 것' 1번 |
| "앱을 설치할 수 없습니다" | `app-armeabi-v7a-release.apk`(19MB)를 대신 보낼 것 |
| 화면 문구·배치 | `app/lib/ui/` 의 해당 탭 파일 |
| 통계 항목 추가/변경 | `domain/statistics.dart` + `ui/stats_tab.dart` |
| 보여줄 회차 수 | `batch/build.py` 의 `APP_BUNDLE_ROUNDS` 하나 |
| 판매점 순위 개수 | `batch/build.py` 의 `RANKING_LIMIT` |
| 갱신이 너무 느리다 | 워크플로 cron. 이미 10분 간격이고 그 이상은 GitHub 쪽 한계 |
...
1. ⚠️ **API 26 호환성이 미검증이다.** 갤럭시 S21(Android 15) 한 대로만 확인했다.
   앱은 Android 8부터 지원한다고 **선언만** 해뒀다.
   사용자가 에뮬레이터 대신 **지인 배포로 확인하기로 했다**(2026-08-09).
   "켜자마자 꺼진다"는 제보가 오면 그때 API 26 AVD를 만들어 재현할 것
