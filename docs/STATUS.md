...
> 최종 갱신: 2026-08-06 / 다음 세션이 이 문서부터 읽으면 된다.
...
| minSdk | 26 (Android 8) | |
| 번들 데이터 | 당첨번호 전 회차 + 판매점 최근 100회차 | 설계 문서 §13-3 |
...
- 판매점 **614 / 1235 회차** — `.\run.ps1 collect` 약 5회 더 필요
- 테스트 **63개** (배치 40 + 앱 23) 전부 통과
- 커밋 7개, 작업 트리 깨끗함
...
- **자동/수동 구분은 262회차부터만 존재한다.** 261회 이하는 `winType`이 전부 0.
...
배치가 `data/draws.json`을 갱신하면 앱 에셋에도 복사해야 한다:
`Copy-Item data\draws.json app\assets\data\draws.json -Force`
