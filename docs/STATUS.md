...
Week 0·1 완료(백필 100%) · **Week 2 진행 중 — 데이터 파이프라인이 GitHub Pages까지 실제로 배포됨. 다음은 ④ 통계 탭**
...
- 테스트 **166개** (배치 57 + 앱 109) 전부 통과, `flutter analyze` 무결점
...
- [ ] **매장 랭킹 육안 확인** — `.\run.ps1 view`에서 같은 매장이 두 줄로
      쪼개져 있는지. `ltShpId` 안정성 검증이며 **테스트로는 판정 불가**.
      백필이 끝나 이제 확인 가능하다. ⑤ 탭 착수 전에 볼 것
