...
- 저장소를 **공개로 전환**했다. 무료 플랜에서는 비공개 저장소로 Pages를 쓸 수
  ...
  덤으로 공개 저장소는 Actions 실행 시간이 무제한이다
- ⏳ **실제 네트워크 동기화는 아직 관측되지 않았다.** 서버 최신(1235)과 앱
  번들 최신(1235)이 같아 `refresh()`가 즉시 빠져나오기 때문이며, 이게 정상
  동작이다. 첫 실제 갱신은 **1236회차가 나오는 토요일 밤**에 일어난다.
  경로 자체는 MockClient 테스트 5건으로 덮여 있다
...
- ⚠️ **`firstWinners == 0`을 '1등 없음'으로 단정하면 안 된다.** 추첨 직후에는
  집계 전이라 갓 나온 회차도 0명으로 보인다. 이걸 확정으로 보고 빈 판매점
  파일을 캐시에 남기면, 당첨금이 확정된 뒤에도 '이미 받았다'며 건너뛴다 —
  **신규 회차의 판매점이 영영 비고 랭킹도 그때부터 멈춘다.**
  2026-08-07에 발견해 고쳤다. 그대로 뒀으면 다음 추첨(1236회)부터 터졌을 것이다.
  총 판매금액으로 가른다(`collect.is_settled_without_winner`) — 앱의
  `Draw.isSettled`와 같은 기준이다
