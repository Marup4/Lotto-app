...
Week 0·1 완료(백필 100%) · **Week 2 진행 중 — 파이프라인이 GitHub Pages까지 배포되고 ④ 통계 탭까지 완료. 남은 화면은 ⑤ 판매점뿐**
...
- 테스트 **186개** (배치 57 + 앱 129) 전부 통과, `flutter analyze` 무결점
...
4. ⑤ 판매점·랭킹 — 착수 가능. **먼저 `.\run.ps1 view`로 랭킹을 육안 확인할 것**
   (같은 가게가 두 줄로 쪼개졌는지. 테스트로는 판정 불가).
   랭킹 1위가 "인터넷 복권판매사이트"(118회, 주소가 `동행복권(dhlottery.co.kr)`)다 —
   실물 매장이 아니라 온라인 구매 채널이므로 화면에서 어떻게 다룰지 정해야 한다
