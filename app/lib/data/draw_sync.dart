...
class DrawSync {
  DrawSync({http.Client? client, String baseUrl = Pages.defaultBase})
  ...
  Future<List<Draw>?> refresh({required int localLatest}) async {
    try {
      final manifest = await _pages.getJson('manifest.json');
      ...
      final body = await _pages.getJson('draws-latest.json');
